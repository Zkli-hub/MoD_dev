    ...
    def forward(
        ...
        *,
        ...
        for lid, layer in enumerate(self.layers):
            ...
            # ======================
            sim_score = F.cosine_similarity(old_h[0], h[0], dim = -1)
            if lid not in self.bi_scores_logger.keys():
                # self.bi_scores_logger[lid] = [torch.mean(sim_score)]
                self.bi_scores_logger[lid] = [sim_score]
            else:
                # self.bi_scores_logger[lid].append(torch.mean(sim_score))
                self.bi_scores_logger[lid].append(sim_score)
            # =========================

        # shape: [b, s, d]
        h = self.norm(h)

        # Report the global sim scores.
        # ======================
        # if len(self.bi_scores_logger[0]) == 2500:
        #     bsim = []
        #     for _k, _v in self.bi_scores_logger.items():
        #         _v = torch.mean(torch.tensor(_v))
        #         print(f'layer_{_k}: {_v}')
        #         bsim.append(_v.item())
        #     print(bsim)
        #     print('------')
        # ======================
