...
class TransformerDecoder(nn.Module):
    ...
    def router_judgement(self, x, previous_x, 
        ...
        if exp_tag == 'sim_score' or exp_tag == 'random':
            ...
            new_judge_score = new_judge_score * (-1)

        involve_seq_len = seq_len
        ...
        if impl == 'topk':
            if exp_tag != 'random':
                if first_inital_flag == False:
                    ...
                    for bid in range(bsz):
                        update_index = update_indices[bid]
                        global_judge_score_record[bid][update_index] = new_judge_score[bid, update_index]
                    _, skip_indices = torch.topk(global_judge_score_record, k = expected_skip_token_num, dim = -1)
                    return skip_indices, expected_skip_token_num, None, None, global_judge_score_record
                else:
                    _, skip_indices = torch.topk(new_judge_score, k = expected_skip_token_num, dim =-1)
                    return skip_indices, expected_skip_token_num, None, None, new_judge_score
            else:
                random_ruler = np.arange(0, involve_seq_len).tolist()
                random.shuffle(random_ruler)
                random_ruler = random_ruler[:expected_skip_token_num]
                skip_indices = torch.tensor([random_ruler], dtype = x.dtype, device = x.device)
                return skip_indices, expected_skip_token_num, None, None, new_judge_score
    ...
    def forward(
        ...
        *,
        ...
        postfix_protected_number = 10 # number of protected postfix-tokens.
        repick_for_sr_ensurance = True
        self.exp_tag = 'position' # ['random', 'sim_score', 'attn_map', 'position']
        ...
        for lid, layer in enumerate(self.layers):
            ...
            if str(lid) in process_cap_map.keys():
                ...
                action_confs = None
                source_hs = h
                pre_attn_score =  temp_ws[lid - 1] if self.exp_tag == 'attn_map' else None
                skip_indices, _, action_confs, layer_loss, global_judge_score_record = self.router_judgement(
                                                        x = temp_hs[lid], previous_x = temp_hs[lid-1], 
                                                        pre_attn_score = pre_attn_score,
                                                        seq_len = seq_len,
                                                        expected_skip_cap = 1 - process_cap_map[str(lid)], 
                                                        layer_idx = lid,
                                                        update_indices = update_indices,
                                                        global_judge_score_record = global_judge_score_record,
                                                        impl = impl_tag, 
                ...
                for bid in range(bsz):
                    ...
                    if prefix_protected_number > 0:
                        remove_condition_prefix = skip_index < prefix_protected_number
                        skip_index = skip_index[~remove_condition_prefix]

                    post_processing_flag = True #if random.uniform(0, 1) > 0.2 else False

                    if seq_len - postfix_protected_number > 0 and post_processing_flag:
                    ...
                    if repick_for_sr_ensurance:
                        rest_distance = int((1 - process_cap_map[str(lid)]) * seq_len) - len(skip_index) # Get the how many tokens be required until now.
                        ...
                        if rest_distance > 0 and (seq_len - prefix_protected_number) >= rest_distance:
                            try:
                                ...
                                if postfix_protected_number > 0 and post_processing_flag:
                                    involve_rechoose_index_flag[-postfix_protected_number:] = 1
                                involve_rechoose_index = torch.where(involve_rechoose_index_flag == 0)[0]
                                involve_rechoose_index, _ = torch.sort(involve_rechoose_index)
                                if self.exp_tag != 'random':
                                    _, rest_topk_index = torch.topk(global_judge_score_record[bid, involve_rechoose_index], k = rest_distance, dim = -1)
                                else:
                                    random_ruler = np.arange(0, global_judge_score_record[bid, involve_rechoose_index].shape[1]).tolist()
                                    random.shuffle(random_ruler)
                                    random_ruler = random_ruler[:rest_distance]
                                    rest_topk_index = torch.tensor([random_ruler], dtype = global_judge_score_record.dtype, device = global_judge_score_record.device)
                                rest_topk_index, _ = torch.sort(rest_topk_index)
                                ...
                                pass
                    skip_index, _ = torch.sort(skip_index)
                    all_actual_cap += ((seq_len - len(skip_index)) / seq_len) # NOTE only for sparse ratio statistic
                ...
                if len(self.sparse_ratio_logger) == 100 or len(self.sparse_ratio_logger) == 200:
                    print(lid, skip_binary_flag)
                    print(global_judge_score_record)
                ...
                # ======================== update =>
                if self.exp_tag != 'attn_map':
                    _updated_h = layer(h, mask = mask, input_pos = input_pos) # NOTE mask and input_pos are `None`.
                else:
                    _updated_h, attnw = layer(h, mask = mask, input_pos = input_pos, return_attn_weights = True)
        ...
        self.sparse_ratio_logger.append(sparse_ratio)

        if len(self.sparse_ratio_logger) % 5000 == 0:
            print(f'{self.exp_tag}, prefix:{prefix_protected_number} postfix:{postfix_protected_number}, Sample {len(self.sparse_ratio_logger)} iters, mean value: {np.mean(self.sparse_ratio_logger)}')
        return output
