...
class FullFinetuneRecipeSingleDevice(FTRecipeInterface):
    ...
    def __init__(self, cfg: DictConfig) -> None:
        ...
        self.custom_global_steps = cfg.custom_global_steps 
        self.cfg = cfg
    ...
    def setup(self, cfg: DictConfig) -> None:
        ...
        # self._ema_module = PostHocEMA(
        #     self._model.skip_router,
        ...
        #     checkpoint_every_num_steps = 10000,
        #     checkpoint_folder = os.path.join(cfg.output_dir, 'post-hoc-ema-checkpoints')  # the folder of saved checkpoints for each sigma_rel (gamma) across timesteps with the hparam above, used to synthesizing a new EMA model after training
        # )
        # self._ema_module = EMA(
        #     self._model.skip_router,
        ...
        #     update_every = 10,          # how often to actually update, to save on compute (updates every 10th
        # )
        # self._model.skip_router.train()
        ...
        if not self._optimizer_in_bwd:
            self._lr_scheduler = self._setup_lr_scheduler(
                ...
                num_training_steps = self.cfg.custom_global_steps,
                last_epoch = -1,
            )
    ...
    def _setup_optimizer(
        ...
        if optimizer_in_bwd:
            ...
            for p in self._model.parameters():
                optim_dict[p] = config.instantiate(cfg_optimizer, [p]) 
    ...
    def save_checkpoint(self, epoch: int) -> None:
        ...
        """
        # synthesized_ema = self._ema_module.synthesize_ema_model(sigma_rel = 0.15)
        # synthesized_ema = self._ema_module

        # with torch.no_grad():
        #     # NOTE: EMA _ema_module restore
        #     self._model.skip_router[0].weight.copy_(synthesized_ema.ema_model[0].weight)
        #     self._model.skip_router[0].bias.copy_(synthesized_ema.ema_model[0].bias)
        #     self._model.skip_router[1].weight.copy_(synthesized_ema.ema_model[1].weight)
        #     self._model.skip_router[1].bias.copy_(synthesized_ema.ema_model[1].bias)
    ...
    def train(self) -> None:
        ...
        for curr_epoch in range(self.epochs_run, self.total_epochs):
            ...
            for idx, batch in enumerate(self._dataloader):
                ...
                self._model.update_training_info(idx, self.custom_global_steps) # update the step info for model
                ...
                # Compute loss
                loss = self._loss_fn(logits, labels) * self.cfg.llm_loss_ratio
                loss_custom = self._model.get_loss()
                if loss_custom != None:
                    loss = loss + loss_custom

                loss = loss / self._gradient_accumulation_steps
                running_loss += loss
                loss.backward()
                self._model.reset_loss()
                ...
                if (idx + 1) % self._gradient_accumulation_steps == 0:
                    if not self._optimizer_in_bwd:
                        ...
                        if self._ema_module != None:
                            self._ema_module.update()
                ...
                if idx == self.custom_global_steps:
                    break
