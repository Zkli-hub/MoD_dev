...
class TransformerDecoder(nn.Module):
    ...
    def __init__(
        ...
        self.custom_loss_fn = nn.BCELoss()

        # NOTE@shared router 4chan.
    ...
    def router_judgement(self, x, previous_x, prefix_protect_id, 
        ...
        # Get sim
        cos_sim = F.cosine_similarity(x[:, prefix_protect_id:, :], previous_x[:, prefix_protect_id:, :], dim = -1) # [bs, v_seq_len]
        ...
        if impl == 'topk':
            ...
            return skip_indices, expected_skip_token_num, None, None
            ...
            if exp_tag == 'shared_router': # NOTE: shared router & input: [cos_sim, sim_index_norm, positions]
                ...
                combined_input = torch.stack((cos_sim, sim_index_norm, attn_std_0, attn_mean_0, attn_std_1, attn_mean_1, positions), dim=-1)
                logits = self.skip_router(combined_input) # go shared router
            ...
            if self.training_mode:
                ...
                skip_indices = []
                for bi in range(bsz):
                    skip_indices.append(torch.where(y_hard[bi, :, 0] == 1)[0]) if y_hard_must else \
                        skip_indices.append(torch.where(y_soft[bi, :, 0] == 1)[0] + prefix_protect_id) # NOTE An useless impl., but better performance????
                ...
                # Get the bce loss
                sim_guided_loss = self.custom_loss_fn(_y_soft, pseudo_skip_binary_flag)
                # Get the sparse loss
    ...
    def forward(
        ...
        *,
        ...
        # skip_cap = 0.80 # expected skip capacity.
        prefix_protect_id = 5 # number of protected prefix-tokens.
        ...
        for lid, layer in enumerate(self.layers):
            ...
            if str(lid) in process_cap_map.keys():
                ...
                @EXP_TAG: str, <Optional>, only works under `impl=='topk'`
                    - shared_router
                    - no_shared_router
                    - shared_router_layer_id

                    NOTE: `y_hard_must` only affects the training behaviour.
                        if `y_hard_must==False`, `skip_indices` is a null set, involed with `y_soft`, 
                            else, `skip_indices` are from the `y_hard` selection (i.e., y_hard = (y_soft > 0.5)).
                """
                skip_indices, _, action_confs, layer_loss = self.router_judgement(temp_hs[lid], temp_hs[lid - 1], 
                                                        prefix_protect_id, seq_len,
                                                        expected_skip_cap = 1 - process_cap_map[str(lid)], 
                                                        layer_idx = lid,
                                                        y_hard_must = False,
                                                        # prompt_attnw = temp_attnws[-1],
                                                        prompt_attnw = None,
                                                        impl = 'learnable', exp_tag = 'shared_router')
                ...
                h_list, process_indices = [], []
                for bid in range(bsz):
                    skip_binary_flag[bid, skip_indices[bid]] = 1
                ...
                h = out_h
                # print(out_h) # To view whether has a grad_fn
            else:
